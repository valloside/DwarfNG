...
static constexpr std::chrono::duration<double, std::milli> msPerFrame(8.33333333);
...
int main()
{
    ...
    WNDCLASSEXW wc = {sizeof(wc), CS_CLASSDC, WndProc, 0L, 0L,
                      ...
                      L"Main Window", nullptr};
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"DwarfNG",
    ...
    while (!done)
    {
        ...
        std::chrono::duration<double, std::milli> delta = std::chrono::system_clock::now() - lastFrame;
        std::this_thread::sleep_for(msPerFrame - delta);
